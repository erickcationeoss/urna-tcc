...
let votoAtual = '';
let modoAdmin = false;
let tempoVotacao = 30; // 30 segundos para votar
let timerInterval;
...
function iniciarTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        if (tempoVotacao <= 0) {
            pararTimer();
            showNotification('Tempo esgotado! Voto em branco registrado automaticamente.', 'warning');
            brancoAutomatico();
        }
    }, 1000);
}
...
function atualizarTimer() {
    ...
    // Alterar aparência conforme o tempo diminui
...
function brancoAutomatico() {
    // Registrar voto em branco automaticamente
    salvarVoto({
    ...
    // Reiniciar timer para próximo voto
    setTimeout(() => {
        iniciarTimer();
    }, 2000);
}
...
function confirma() {
    ...
    // Pequeno delay antes de resetar para o próximo voto
    setTimeout(() => {
        corrige();
        iniciarTimer();
    }, 2000);
